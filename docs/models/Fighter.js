...
export class Fighter {
  ...
  constructor(config = {}) {
    ...
    this.contracts = {
       ...
       * or null if unsigned. Set by acceptLeagueOffer'ing a pending offer
       * below (see engine/LeagueEngine.js#acceptLeagueOffer) — binds this
       * fighter to that organization until fightsRemaining reaches 0
       * (consumed one at a time by consumeExclusivityFight(), called once
       * per resolved fight) or the release clause is paid
       * (releaseExclusivityContract()).
       */
      exclusivity: config.contracts?.exclusivity ? { ...config.contracts.exclusivity } : null,
      /**
       * V4.0 "Offres de Contrat Recues": unsolicited league-roster offers
       * this fighter has been sent — { orgId, offeredOnDay, terms:
       * {fightsRequired, pursePerFight, signingBonus, releaseClauseCost} }.
       * Generated by engine/LeagueEngine.js#evaluateLeagueOffers (a win
       * streak or enough gym Hype triggers one), resolved by
       * acceptLeagueOffer/declineLeagueOffer — the manager no longer
       * applies to a league, the league reaches out first.
       */
  ...
  /**
   * V4.0 "Offres de Contrat Recues": records an unsolicited league-roster
   * offer sent to this fighter — see engine/LeagueEngine.js
   * #evaluateLeagueOffers, the only caller. A no-op if one from the same
   * org is already pending (never duplicates or refreshes an existing
   * offer's terms/day).
   * @param {string} orgId
   * @param {{ fightsRequired: number, pursePerFight: number, signingBonus: number, releaseClauseCost: number }} terms
   * @param {number} offeredOnDay
   */
  receiveLeagueOffer(orgId, terms, offeredOnDay) {
    if (this.hasPendingOffer(orgId)) return;
    this.contracts.pendingOffers.push({ orgId, offeredOnDay, terms: { ...terms } });
  }

  /** @returns {boolean} True if a pending offer from this org is currently awaiting a decision. */
  hasPendingOffer(orgId) {
    return this.contracts.pendingOffers.some((offer) => offer.orgId === orgId);
  }

  /** @returns {Object|undefined} The pending offer from this org, or undefined. */
  getPendingOffer(orgId) {
    return this.contracts.pendingOffers.find((offer) => offer.orgId === orgId);
  }

  /** @returns {Object[]} Every pending offer this fighter currently holds. */
  getPendingOffers() {
    return [...this.contracts.pendingOffers];
  }

  /** Removes the pending offer from this org (accepted or declined) — a no-op if none exists. @returns {boolean} True if an offer was actually removed. */
  clearPendingOffer(orgId) {
    const before = this.contracts.pendingOffers.length;
    this.contracts.pendingOffers = this.contracts.pendingOffers.filter((offer) => offer.orgId !== orgId);
    return this.contracts.pendingOffers.length < before;
  }
